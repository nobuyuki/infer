...
type compiler = Kotlin | Kotlinc [@@deriving compare]
...
let compile compiler build_prog build_args =
  let prog, prog_args =
    ...
    | Kotlin, Some jar ->
        (build_prog, [jar])     (* kotlin app.jar *)
    | _, Some jar ->
        (* fall back to java in PATH to avoid passing -jar to kotlinc *)
        ("kotlin", [jar])
  in
  let cli_args, file_args =
    let args = "-Xreport-output-files" :: "-d" :: Config.javac_classes_out :: build_args in
...
(*  let args_file =
  ...
  in
  let fallback () = try_run ("kotlinc" :: cli_file_args) None in
...
let capture compiler ~prog ~args =
  ...
  if Config.analyzer <> Config.CompileOnly then KMain.from_verbose_out verbose_out_file ;
