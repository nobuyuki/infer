...
val create_callee_attributes :
  Tenv.t -> KClasspath.program -> JBasics.class_name -> JBasics.method_signature -> Typ.Procname.t
  -> ProcAttributes.t option

val create_am_procdesc :
  SourceFile.t -> KClasspath.program -> KContext.icfg -> Javalib.abstract_method -> Typ.Procname.t
  -> Procdesc.t
(** Create the procedure description for an abstract method *)

val create_native_procdesc :
  SourceFile.t -> KClasspath.program -> KContext.icfg -> JCode.jcode Javalib.concrete_method
  -> Typ.Procname.t -> Procdesc.t
(** Create the procedure description for a concrete method *)

val create_empty_procdesc :
  SourceFile.t -> KClasspath.program -> Printer.LineReader.t -> KContext.icfg
...
val instruction : KContext.t -> int -> JBir.instr -> translation
