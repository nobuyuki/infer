...
let add_edges (context: KContext.t) start_node exn_node exit_nodes method_body_nodes impl
    super_call =
  ...
  let rec get_body_nodes_ pc visited =
    ...
    | KTrans.Skip when is_last pc && not (KContext.is_goto_jump context pc) ->
        exit_nodes
    | KTrans.Skip ->
        direct_successors pc (Int.Set.add visited pc)
    | KTrans.Instr node ->
        [node]
    | KTrans.Prune (node_true, node_false) ->
        [node_true; node_false]
    | KTrans.Loop (join_node, _, _) ->
        [join_node]
  and direct_successors pc visited =
    if is_last pc && not (KContext.is_goto_jump context pc) then exit_nodes
    else
      match KContext.get_goto_jump context pc with
      | KContext.Next ->
          let next_pc = pc + 1 in
          if Int.Set.mem visited next_pc then [] else get_body_nodes_ next_pc visited
      | KContext.Jump goto_pc when Int.Set.mem visited goto_pc ->
          [] (* loop in goto *)
      | KContext.Jump goto_pc ->
  ...
  let get_succ_nodes node pc =
    match KContext.get_if_jump context node with
    | None ->
  ...
  let get_exn_nodes =
    ...
    else KTransExn.create_exception_handlers context [exn_node] get_body_nodes impl
  in
  let connect node pc =
  ...
  let connect_nodes pc translated_instruction =
    ...
    | KTrans.Loop (join_node, node_true, node_false) ->
...
let add_cmethod source_file program linereader icfg cm proc_name =
  ...
  if Inferconfig.skip_implementation_matcher source_file proc_name then
    ignore (KTrans.create_empty_procdesc source_file program linereader icfg cm proc_name)
  else
    match KTrans.create_cm_procdesc source_file program linereader icfg cm proc_name with
    | None ->
        ()
    | Some (procdesc, start_node, exit_node, exn_node, jbir_code) ->
        let context = KContext.create_context icfg procdesc jbir_code cn source_file program in
        let method_body_nodes = Array.mapi ~f:(KTrans.instruction context) (JBir.code jbir_code) in
...
let path_of_cached_classname cn =
  ...
  Filename.concat package_path (JBasics.cn_simple_name cn ^ ".kt")
...
let create_icfg source_file linereader program icfg cn node =
  ...
  let translate m =
    let proc_name = KTransType.translate_method_name m in
    KClasspath.set_callee_translated program proc_name ;
    if KClasspath.is_model proc_name then
      ...
      try
        ...
        | Javalib.AbstractMethod am ->
            ignore (KTrans.create_am_procdesc source_file program icfg am proc_name)
        | Javalib.ConcreteMethod cm when KTrans.is_java_native cm ->
            ignore (KTrans.create_native_procdesc source_file program icfg cm proc_name)
...
let should_capture classes package_opt source_basename node =
  ...
  let match_package pkg cn =
    match KTransType.package_to_string (JBasics.cn_package cn) with
    | None ->
        String.equal pkg ""
    | Some found_pkg ->
...
let compute_source_icfg linereader classes program tenv source_basename package_opt source_file =
  let icfg = {KContext.cfg= Cfg.create (); tenv} in
  ...
  let () =
    JBasics.ClassMap.iter
      ...
      (KClasspath.get_classmap program)
  in
  icfg.KContext.cfg
