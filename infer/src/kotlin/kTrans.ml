...
let get_field_name program static tenv cn fs =
  let {Typ.Struct.fields; statics} = KTransType.get_class_struct_typ program tenv cn in
  match
    List.find
...
let formals_from_signature program tenv cn ms kind =
  ...
  let collect l vt =
    ...
    let arg_type = KTransType.value_type program tenv vt in
    (arg_name, arg_type) :: l
  in
  let init_arg_list =
    match kind with
    ...
    | Typ.Procname.Java.Non_Static ->
        [(KConfig.this, KTransType.get_class_type program tenv cn)]
  in
...
let translate_formals program tenv cn impl =
  let collect l (vt, var) =
    ...
    let typ = KTransType.param_type program tenv cn var vt in
    (name, typ) :: l
  in
    ...
    | Some variable_table ->
        List.fold
          ~f:(fun accu (_, _, var_name, var_type, _) ->
            ...
            and typ = KTransType.value_type program tenv var_type in
...
let get_constant (c: JBir.const) =
  ...
  | `Class ot ->
      Const.Cclass (Ident.string_to_name (KTransType.object_type_to_string ot))
  | `Double f ->
      Const.Cfloat f
  | `Float f ->
      Const.Cfloat f
...
let is_clone ms = String.equal (JBasics.ms_name ms) KConfig.clone_name
...
let create_callee_attributes tenv program cn ms procname =
  let f jclass =
    try
      ...
      let formals =
        formals_from_signature program tenv cn ms (KTransType.get_method_kind jmethod)
      in
      let ret_type = KTransType.return_type program tenv ms in
      let access, method_annotation, exceptions, is_abstract =
        match jmethod with
        | Javalib.AbstractMethod am ->
            ( trans_access am.Javalib.am_access
            , KAnnotation.translate_method am.Javalib.am_annotations
            , List.map ~f:JBasics.cn_name am.Javalib.am_exceptions
            , true )
        | Javalib.ConcreteMethod cm ->
            ( trans_access cm.Javalib.cm_access
            , KAnnotation.translate_method cm.Javalib.cm_annotations
    ...
    with Not_found -> None
  in
  Option.bind ~f (KClasspath.lookup_node cn program)
...
let create_am_procdesc source_file program icfg am proc_name : Procdesc.t =
  let tenv = icfg.KContext.tenv in
  let m = Javalib.AbstractMethod am in
  let cn, ms = JBasics.cms_split (Javalib.get_class_method_signature m) in
  let formals = formals_from_signature program tenv cn ms (KTransType.get_method_kind m) in
  let method_annotation = KAnnotation.translate_method am.Javalib.am_annotations in
  let procdesc =
    let proc_attributes =
      ...
      ; method_annotation
      ; ret_type= KTransType.return_type program tenv ms
      ; loc= Location.none source_file }
    in
    Cfg.create_proc_desc icfg.KContext.cfg proc_attributes
  in
...
let create_native_procdesc source_file program icfg cm proc_name =
  ...
  let cn, ms = JBasics.cms_split (Javalib.get_class_method_signature m) in
  let formals = formals_from_signature program tenv cn ms (KTransType.get_method_kind m) in
  let method_annotation = KAnnotation.translate_method cm.Javalib.cm_annotations in
  let procdesc =
    let proc_attributes =
      { (ProcAttributes.default proc_name) with
...
let create_empty_procdesc source_file program linereader icfg cm proc_name =
  ...
  let proc_attributes =
    ...
    ; ret_type= KTransType.return_type program tenv ms }
  in
  let proc_desc = Cfg.create_proc_desc icfg.KContext.cfg proc_attributes in
...
let create_cm_procdesc source_file program linereader icfg cm proc_name =
  ...
  try
    ...
    KContext.add_exn_node proc_name exn_node ;
    Procdesc.set_start_node procdesc start_node ;
    Procdesc.set_exit_node procdesc exit_node ;
...
let rec expression (context: KContext.t) pc expr =
  let program = context.program in
  let loc = get_location context.source_file context.impl pc in
  let tenv = KContext.get_tenv context in
  let type_of_expr = KTransType.expr_type context expr in
  let trans_var pvar =
  ...
  match expr with
  | JBir.Var (_, var) ->
      let pvar = KContext.set_pvar context var type_of_expr in
      trans_var pvar
  | JBir.Const c -> (
    match c with
    (* We use the constant <field> internally to mean a variable. *)
    | `String s
      when String.equal (JBasics.jstr_pp s) KConfig.field_cst ->
        let varname = KConfig.field_st in
  ...
  | JBir.Unop (unop, ex)
    -> (
      let type_of_ex = KTransType.expr_type context ex in
      ...
      | JBir.Conv conv ->
          let cast_ex = Exp.Cast (KTransType.cast_type conv, sil_ex) in
      ...
      | JBir.InstanceOf ot | JBir.Cast ot ->
          let subtypes =
            ...
            | _ ->
                assert false
          in
          let sizeof_expr = KTransType.sizeof_of_object_type program tenv ot subtypes in
          let builtin =
            match unop with
            | JBir.InstanceOf _ ->
  ...
  | JBir.Field (ex, cn, fs) ->
      ...
      let sil_type = KTransType.get_class_type_no_pointer program tenv cn in
  ...
  | JBir.StaticField (cn, fs) ->
      ...
      let sil_type = KTransType.get_class_type_no_pointer program tenv cn in
      if KTransType.is_autogenerated_assert_field field_name then
...
let method_invocation (context: KContext.t) loc pc var_opt cn ms sil_obj_opt expr_list invoke_code
    method_kind =
  ...
  let resolve_method (context: KContext.t) cn ms =
    let rec loop fallback_cn cn =
      match KClasspath.lookup_node cn context.program with
      | None ->
          fallback_cn
      | Some node ->
  ...
  let callee_procname =
    ...
    if JBasics.cn_equal cn' (JBasics.make_cn KConfig.infer_builtins_cl)
       && BuiltinDecl.is_declared proc
    then proc
    else KTransType.get_method_procname cn' ms method_kind
  in
  KClasspath.add_missing_callee program callee_procname cn' ms ;
  let call_instrs =
    ...
    let return_type =
      ...
      | None ->
          Typ.mk Tvoid
      | Some vt ->
          KTransType.value_type program tenv vt
    in
    ...
    | None ->
        ...
        instrs @ [call_instr]
    | Some var ->
        let sil_var = KContext.set_pvar context var return_type in
  ...
  let instrs =
    ...
    | _
      when Config.models_mode || KClasspath.is_model callee_procname ->
        call_instrs
...
let is_this expr =
  ...
  | JBir.Var (_, var) -> (
    ...
    | None ->
        false
    | Some name_opt ->
        String.equal (Mangled.to_string KConfig.this) name_opt )
  | _ ->
      false
...
let instruction (context: KContext.t) pc instr : translation =
  let tenv = KContext.get_tenv context in
  let program = context.program in
  ...
  try
    ...
    | JBir.AffectVar (var, expr) ->
        ...
        let pvar = KContext.set_pvar context var sil_type in
    ...
    | JBir.Return expr_option ->
        ...
        in
        KContext.add_goto_jump context pc KContext.Exit ;
        Instr node
    ...
    | JBir.AffectField (e_lhs, cn, fs, e_rhs) ->
        ...
        let field_name = get_field_name program false tenv cn fs in
        let type_of_the_surrounding_class = KTransType.get_class_type_no_pointer program tenv cn in
    ...
    | JBir.AffectStaticField (cn, fs, e_rhs) ->
        ...
        Instr node
    | JBir.Goto goto_pc ->
        KContext.reset_pvar_type context ;
        KContext.add_goto_jump context pc (KContext.Jump goto_pc) ;
        Skip
    | JBir.Ifd ((op, e1, e2), if_pc) ->
        ...
        in
        KContext.add_if_jump context prune_node_false if_pc ;
    ...
    | JBir.New (var, cn, constr_type_list, constr_arg_list) ->
        ...
        let class_type = KTransType.get_class_type program tenv cn in
        let class_type_np = KTransType.get_class_type_no_pointer program tenv cn in
        let sizeof_exp =
          Exp.Sizeof
        ...
        in
        let constr_ms = JBasics.make_ms KConfig.constructor_name constr_type_list None in
        let constr_procname, call_instrs =
          ...
          method_invocation context loc pc None cn constr_ms ret_opt constr_arg_list I_Special
            Typ.Procname.Java.Non_Static
        in
        let pvar = KContext.set_pvar context var class_type in
    ...
    | JBir.NewArray (var, vt, expr_list) ->
        ...
        let content_type = KTransType.value_type program tenv vt in
        let array_type = KTransType.create_array_type content_type (List.length expr_list) in
        let array_name = KContext.set_pvar context var array_type in
    ...
    | JBir.InvokeVirtual (var_opt, obj, call_kind, ms, args)
      -> (
        ...
        let trans_virtual_call original_cn invoke_kind =
          let cn' =
            match KTransType.extract_cn_no_obj sil_obj_type with
            | Some cn ->
                cn
            | None ->
                original_cn
    ...
    | JBir.Check JBir.CheckNullPointer expr when Config.tracing ->
        ...
        let throw_npe_node =
          ...
          and npe_cn = JBasics.make_cn KConfig.npe_cl in
          let class_type = KTransType.get_class_type program tenv npe_cn
          and class_type_np = KTransType.get_class_type_no_pointer program tenv npe_cn in
          let sizeof_exp =
            Exp.Sizeof
              {typ= class_type_np; nbytes= None; dynamic_length= None; subtype= Subtype.exact}
          in
          ...
          let new_instr =
            Sil.Call (Some (ret_id, class_type), builtin_new, args, loc, CallFlags.default)
          in
          let constr_ms = JBasics.make_ms KConfig.constructor_name [] None in
          let _, call_instrs =
            let ret_opt = Some (Exp.Var ret_id, class_type) in
    ...
    | JBir.Check JBir.CheckArrayBound (array_expr, index_expr) when Config.tracing ->
        ...
        and throw_out_of_bound_node =
          ...
          in
          let out_of_bound_cn = JBasics.make_cn KConfig.out_of_bound_cl in
          let class_type = KTransType.get_class_type program tenv out_of_bound_cn
          and class_type_np = KTransType.get_class_type_no_pointer program tenv out_of_bound_cn in
          let sizeof_exp =
            Exp.Sizeof
              {typ= class_type_np; nbytes= None; dynamic_length= None; subtype= Subtype.exact}
          in
    ...
    | JBir.Check JBir.CheckCast (expr, object_type) when Config.tracing ->
        let sil_type = KTransType.expr_type context expr
        and instrs, sil_expr, _ = expression context pc expr
        and ret_id = Ident.create_fresh Ident.knormal
        and sizeof_expr =
          KTransType.sizeof_of_object_type program tenv object_type Subtype.subtypes_instof
        in
        ...
        and throw_cast_exception_node =
          ...
          and cce_cn = JBasics.make_cn KConfig.cce_cl in
          let class_type = KTransType.get_class_type program tenv cce_cn
          and class_type_np = KTransType.get_class_type_no_pointer program tenv cce_cn in
          let sizeof_exp =
            Exp.Sizeof
              {typ= class_type_np; nbytes= None; dynamic_length= None; subtype= Subtype.exact}
          in
