...
let const_type const =
  match const with
  | `String _ ->
      JBasics.TObject (JBasics.TClass (JBasics.make_cn KConfig.string_cl))
  | `Class _ ->
      JBasics.TObject (JBasics.TClass (JBasics.make_cn KConfig.class_cl))
  | `Double _ ->
...
and object_type_to_string' ot =
  ...
  | JBasics.TClass class_name ->
      KConfig.class_code (JBasics.cn_name class_name)
  | JBasics.TArray vt ->
...
let method_signature_names ms =
  let return_type_name =
    ...
    | None ->
        if String.equal (JBasics.ms_name ms) KConfig.constructor_name then None
        else Some (None, KConfig.void)
    | Some vt ->
...
let create_sil_class_field cn {Javalib.cf_signature; cf_annotations; cf_kind} =
  ...
  and annotation =
    let real_annotations = KAnnotation.translate_item cf_annotations in
...
let collect_interface_field cn inf l =
  ...
  let annotation = KAnnotation.translate_item inf.Javalib.if_annotations in
...
let add_model_fields program classpath_fields cn =
  ...
  in
  try
    match JBasics.ClassMap.find cn (KClasspath.get_models program) with
...
let rec get_all_fields program tenv cn =
  ...
  let trans_fields classname =
    match KClasspath.lookup_node classname program with
...
and get_class_struct_typ program tenv cn =
  ...
  | Some struct_typ ->
      struct_typ
  | None ->
    match KClasspath.lookup_node cn program with
    | None ->
    ...
    | Some node ->
        ...
        let supers, fields, statics, annots =
          ...
          | Javalib.JInterface jinterface ->
              ...
              let item_annotation = KAnnotation.translate_item jinterface.Javalib.i_annotations in
          ...
          | Javalib.JClass jclass ->
              ...
              in
              let item_annotation = KAnnotation.translate_item jclass.Javalib.c_annotations in
...
let param_type program tenv cn name vt =
  if String.equal (JBir.var_name_g name) (Mangled.to_string KConfig.this) then
    get_class_type program tenv cn
  else value_type program tenv vt


let get_var_type_from_sig (context: KContext.t) var =
...
let get_var_type context var =
  let typ_opt = KContext.get_var_type context var in
...
let rec expr_type (context: KContext.t) expr =
  let program = context.program in
  let tenv = KContext.get_tenv context in
  match expr with
  | JBir.Const const ->
