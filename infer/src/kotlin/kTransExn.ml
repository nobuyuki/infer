...
let translate_exceptions (context: KContext.t) exit_nodes get_body_nodes handler_table =
    ...
    try ignore (Hashtbl.find catch_block_table handler_list) with Not_found ->
      let collect succ_nodes rethrow_exception handler =
        ...
        | Some exn_class_name ->
            let exn_type =
              match
                KTransType.get_class_type context.program (KContext.get_tenv context)
                  exn_class_name
              with
              | {Typ.desc= Tptr (typ, _)} ->
                  typ
            ...
            let instr_set_catch_var =
              let catch_var = KContext.set_pvar context handler.JBir.e_catch_var ret_type in
