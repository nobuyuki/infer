...
let init_global_state source_file =
  ...
  Language.curr_language := Language.Kotlin ;
  DB.Results_dir.init source_file ;
  Ident.NameGenerator.reset () ;
  KContext.reset_exn_node_table ()
...
let do_source_file linereader classes program tenv source_basename package_opt source_file =
  ...
  let cfg =
    KFrontend.compute_source_icfg linereader classes program tenv source_basename package_opt
      source_file
  in
...
let capture_libs linereader program tenv =
  let capture_class tenv cn node =
    ...
    | Javalib.JInterface _ ->
        ()
    | Javalib.JClass _ when KFrontend.is_classname_cached cn ->
        ()
    | Javalib.JClass _ ->
        let fake_source_file = SourceFile.from_abs_path (KFrontend.path_of_cached_classname cn) in
        init_global_state fake_source_file ;
        let cfg = KFrontend.compute_class_icfg fake_source_file linereader program tenv node in
        store_icfg fake_source_file cfg ; KFrontend.cache_classname cn
  in
  JBasics.ClassMap.iter (capture_class tenv) (KClasspath.get_classmap program)
...
let store_callee_attributes tenv program =
  let f proc_name cn ms =
    Option.iter ~f:Attributes.store (KTrans.create_callee_attributes tenv program cn ms proc_name)
  in
  KClasspath.iter_missing_callees program ~f
...
let do_all_files classpath sources classes =
  ...
  let program = KClasspath.load_program classpath classes in
  ...
  String.Map.iteri
    ~f:(fun ~key:basename ~data:file_entry ->
      ...
      | KClasspath.Singleton source_file ->
          translate_source_file basename (None, source_file) source_file
      | KClasspath.Duplicate source_files ->
          List.iter
...
let main load_sources_and_classes =
  ...
  | false, true ->
      KClasspath.add_models Config.models_jar ) ;
  ...
  let classpath, sources, classes =
    ...
    | `FromVerboseOut verbose_out_file ->
        Printf.printf "FromVerboseOut %s" verbose_out_file;
        KClasspath.load_from_verbose_output verbose_out_file
    | `FromArguments path ->
        Printf.printf "FromArguments %s" path;
        KClasspath.load_from_arguments path
  in
  if String.Map.is_empty sources then L.(die UserError) "Failed to load any Kotlin source code"
  else do_all_files classpath sources classes


let from_arguments path = main (`FromArguments path)

let from_verbose_out verbose_out_file = Printf.printf "TOMIZAWA: %s" verbose_out_file ; main (`FromVerboseOut verbose_out_file)
