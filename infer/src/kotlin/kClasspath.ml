...
type mode = MOutput | MSources | MOther

let load_from_verbose_output kotlinc_verbose_out =
  Printf.printf "OUTFILE : %s" kotlinc_verbose_out;
  let file_in = In_channel.create kotlinc_verbose_out in
  let mode = ref MOther in
  let rec loop paths roots sources classes =
    try
      let line = In_channel.input_line_exn file_in in
      if String.equal (String.sub line ~pos:0 ~len:7) "output:" then
        (mode := MOutput;
         loop paths roots sources classes)
      else if String.equal (String.sub line ~pos:0 ~len:8) "Sources:" then
        (mode := MSources;
         loop paths roots sources classes)
      else
        match !mode with
        ...
        | MSources ->
            loop paths roots (add_source_file line sources) classes
        | MOther ->
            loop paths roots sources classes            
    with
